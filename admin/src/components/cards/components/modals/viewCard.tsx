...
const ViewCard:React.ForwardRefRenderFunction<ModalProps,Props> =({item,runMutate,refresh},ref) => {
  ...
  const [isEditMode, setIsEdidtMode] = useState(false)
  const [categoryOptions, setCategoryOptions] = useState([])
  const [selectedCard, setSelectedCard] = useState({status:false,slug:'',cardOrientation:'',title:'',image:{},description:'',tags:[],category:[],subCategory:[],subCategoryChildren:[]});
  ...
  useEffect(()=>{
    ...
    const {title,slug,description,status,image,orientation,category,subCategory,tags,subCategoryChildren} = item;        
    ...
    if(isModalVisible||initialFormData){
            ...
            if(subCategory.length){
              const selectedsubCategoryOption = subCategoryData? subCategoryData.subCategories.filter(sub=>subCategory.map(el=>el._id).includes(sub._id)):[]
    ...
    },[newTags,isModalVisible,initialFormData])
                ...
                <Form
                    ...
                    <div className='cardImage'>
                        <Form.Item 
                          ...
                          <ImageUpload uploadedImage={getImage} image={item?item.image.path:''} />
  ...
  const View = ()=>{
      ...
      const categories = item.category.map((cat,i,{length})=>{
        if(i!==length-1){
          return cat.name+' , '
        }
        return cat.name
      ...
      const subCategories = item.subCategory.map((sub,i,{length})=>{
        if(i!==length-1){
          return sub.name+' , '
        }
        return sub.name
      ...
      const subCategoryChildren = item.subCategoryChildren.map((child,i,{length})=>{
        if(i!==length-1){
          return child.name+' , '
        }
        return child.name
          ...
          <div className="card-container" ref={refView} >
              ...
              <div className='card-items'>
                <ul>
                  ...
                  <li><span className='inline-input'>Card Orientation:</span><span className='item-value'>{item.orientation.charAt(0).toUpperCase() + item.orientation.slice(1)}</span></li>
                  ...
                  <li><span className='inline-input'>Description:</span><span className='item-value'>{item?.description}</span></li>
                  <li><span className='inline-input'>Tags:</span>
                    <span className='item-value'>
                      {item.tags&&item.tags.length?item.tags.map((item)=>(<Tag color="magenta" key={item._id}>{item.name}</Tag>)):<span className='no-data'>no data associated</span>}
                    </span></li>
                </ul>
              </div>
          </div>
